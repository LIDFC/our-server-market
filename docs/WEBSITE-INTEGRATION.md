...
Выдавать посылки, выполнять команды сервера, читать инвентарь, привязывать сундук. Всё это делает плагин, и он же
решает, можно ли.

Лот сайт создать может — но только из привязанного сундука и только вместе с отпечатком, который он получил, читая
этот сундук. Предметы при этом двигает плагин, на своём главном потоке, и сверяет план ещё раз по живому сундуку перед
тем, как что-то взять.

Бэкенд сайта подставляет UUID из своей базы по сессии; UUID из тела запроса браузера не берётся нигде и никогда.

## 10. Проверка связи

```bash
curl -s -H "Authorization: Bearer $MARKET_API_TOKEN" "$MARKET_API_URL/health"   # {"ok":true,"schema":2}
