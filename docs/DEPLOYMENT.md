...
В логе при старте видно версию схемы:

```
[OurServerMarket] Database ready at market.db, schema version 2
```

Миграции применяются сами, каждая в своей транзакции. Второй шаг добавляет склады и одну колонку со значением по
умолчанию — лоты, сделки и посылки он не трогает.
...
Сундук — единственное место, где рынок трогает мир, и единственное, что нельзя проверить в CI: MockBukkit не умеет ни
чанков, ни persistent data. Поэтому проверка руками, вторым аккаунтом.

1. Посмотрите на сундук, `/market chest` — «привязан как ваш склад». `/market chest info` показывает координаты,
   размер и сколько занято слотов.
2. Вторым аккаунтом попробуйте открыть этот сундук и сломать его — оба раза отказ с сообщением.
3. Поставьте воронку под сундук и положите в неё что-нибудь сверху: из склада воронка ничего не вытягивает, внутрь
   тоже ничего не кладёт.
4. Вторым аккаунтом поставьте обычный сундук вплотную к привязанному одинарному — блок не ставится.
5. Взорвите рядом TNT: соседние блоки разлетаются, склад остаётся.
6. На сайте, в разделе «Рынок» → «Мой склад», содержимое совпадает с игрой. Выложите лот со страницы — вещи уходят из
   сундука, лот появляется на доске.
7. Выйдите из игры и повторите пункт 6: со складом всё работает и офлайн, ради этого всё и затевалось.
8. Переложите что-нибудь в сундуке в игре, не обновляя страницу, и нажмите «Выложить лот» — отказ «содержимое
   изменилось», ничего не взято.
9. Падение: выложите лот со страницы и сразу `pkill -9 -f server.jar`, не дожидаясь автосейва. После старта
   `/market chest info` и `/market mine` должны описывать одно и то же состояние, а вещь существовать в одном
   экземпляре:

```sql
SELECT ts, from_holder, to_holder, amount FROM item_movements WHERE item_uid = '…' ORDER BY id;
```
